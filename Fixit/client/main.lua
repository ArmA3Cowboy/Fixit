...
local cooldownTime = 600000  -- 10 minutes in milliseconds
...
RegisterNUICallback('callRepair', function(data, cb)
    local currentTime = GetGameTimer()
    if currentTime - lastRepairTime < cooldownTime then
        local remainingMs = cooldownTime - (currentTime - lastRepairTime)
        local message = "⏰ Please wait " .. math.ceil(remainingMs / 60000) .. " minute(s) before calling for repair again."
        notify(message)
        SendNUIMessage({
            type = 'cooldown',
    ...
    spawnTowTruck()
    CloseNUI()  -- Close the NUI after calling repair
    cb('ok')
end)
...
RegisterKeyMapping('opennui', 'Open Fixit NUI', 'keyboard', 'F4')
